...
# number of running
  ...
  for (j in 1:ncal){
    ...
    for (k in 1:nrow(para_cal)) {
      ...
      newvalue<-as.numeric(param_cal_ranges_lh[j,k])
      ...
      file.copy(watout_files,Calb_lh,overwrite = TRUE)
      file.rename("../Calb_lh/watout.dat", paste("../Calb_lh",paste0(watout_files,"_",j),sep='/'))
      
      ####################################################################################
       #  #1) reset the parameter files-------------
       # setwd(Backup)
       ...
       #2) reset the parameter files-------------
        ...
        # start.time <- Sys.time()
        # dir_copy(Backup_params,TxtInOut, overwrite = TRUE)
        # end.time <- Sys.time()
        # time.taken <- end.time - start.time
        # 
        # copy directory
 ...
 ###############################################################################
  ...
  swat_calb<-function(ncal,tcal,para_cal){
